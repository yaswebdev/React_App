...
const Login = ({ onLogin }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (username === 'yassine' && password === '12345') {
      setIsLoggedIn(true); // Set isLoggedIn to true upon successful login
      setTimeout(() => {
        setIsLoggedIn(false); // Reset isLoggedIn after showing success message (simulated)
        setUsername('');
        setPassword('');
      ...
      <div className="max-w-md w-full space-y-8">
        ...
        {isLoggedIn && (
          <div className="bg-green-100 border border-green-400 text-green-700 px-4 py-2 rounded relative" role="alert">
            <strong className="font-bold">Success:</strong> Login successful!
          </div>
        )}
